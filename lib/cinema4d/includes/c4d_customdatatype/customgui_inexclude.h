...
#define IN_EXCLUDE_DATA_SELECTION	10

#define IN_EXCLUDE_FLAG_INIT_STATE				 2065	///< Int32 The initial state of a new element.
#define IN_EXCLUDE_FLAG_SEND_SELCHANGE_MSG 2066	///< Bool

#define IN_EXCLUDE_FLAG_NUM_FLAGS		 2000				///< Int32 The number of flags.
...
//----------------------------------------------------------------------------------------
/// InExclude data type (@ref CUSTOMDATATYPE_INEXCLUDE_LIST).
/// @addAllocFreeNote
///
/// <b>Example</b>: <i>Accessing include/exclude object list in light object:</i>
/// @code
/// GeData data;
...
public:
	...
	/// @return												@trueIfOtherwiseFalse{@formatParam{pObject} was successfully inserted into the list}
	//----------------------------------------------------------------------------------------
	...
	/// @return												@trueIfOtherwiseFalse{@formatParam{pObject} was successfully removed from the list}
	//----------------------------------------------------------------------------------------
	Bool DeleteObject(BaseDocument* doc, BaseList2D* pObject)
	{
		return DeleteObject(GetObjectIndex(doc, pObject));
	}

	//----------------------------------------------------------------------------------------
	/// Removes the object at index @formatParam{lIndex} in the list.
	/// @param[in] lIndex							The object index. 0 <= @formatParam{lIndex} < GetObjectCount()
	/// @return												@trueIfOtherwiseFalse{the object at index @formatParam{lIndex} was successfully removed from the list}
	//----------------------------------------------------------------------------------------
	...
	/// @return												The object count in the list.
	//----------------------------------------------------------------------------------------
	...
	//----------------------------------------------------------------------------------------
	/// Sets set flags for the object at index @formatParam{lIndex} in the list.
	/// @param[in] lIndex							The object index. 0 <= @formatParam{lIndex} < GetObjectCount()
	...
	{
		return GetFlags(GetObjectIndex(doc, pObject));
	}

	//----------------------------------------------------------------------------------------
	/// Gets the data container for the object at index @formatParam{lIndex} in the list.
	/// @param[in] lIndex							The object index. 0 <= @formatParam{lIndex} < GetObjectCount()
	/// @return												The data container. @theOwnsPointed{inexclude list,container}
	//----------------------------------------------------------------------------------------
	BaseContainer* GetData(Int32 lIndex) const;

	//----------------------------------------------------------------------------------------
	/// Gets the data container for @formatParam{pObject}.
	/// @param[in] doc								The document to evaluate the links in. @callerOwnsPointed{document}
	...
	/// @param[in] doc								The document to evaluate the links in. @callerOwnsPointed{document}
	/// @param[in] hierarchy_bit			The bit position to generate the table for, starts at @em 0.
	/// @return												The built inclusion table, or @formatConstant{nullptr}. @callerOwnsPointed{inclusion table}
	/// @warning	The returned inclusion table has to be freed with @ref FreeInclusionTable().
	//----------------------------------------------------------------------------------------
...
public:
	...
	/// @return												The object count in the table.
	//----------------------------------------------------------------------------------------
	Int32 GetObjectCount();

	//----------------------------------------------------------------------------------------
	/// Gets an object in the table at index @formatParam{lIndex}.
	/// @param[in] lIndex							An object index. @em 0 <= @formatParam{lIndex} < GetObjectCount()
	/// @return												The object at index @formatParam{lIndex}, or @formatConstant{nullptr}.
	//----------------------------------------------------------------------------------------
...
/// @param[in,out] pTable					@theToDestruct{inclusion table}
//----------------------------------------------------------------------------------------
