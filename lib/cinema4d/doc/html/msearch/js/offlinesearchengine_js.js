/*++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	var JSSearchEngine = {
		...
		init : function(start, end, uhg, s2s) {
			if (!(this.pEl = document.getElementById("searchresult")) || !(this.searchForm = document.forms["searchengine"]))
				return false;
			...
			this.searchForm.onsubmit = function(e) {
				...
				if (this.Instanz.keyWords.length == 0){
					window.alert("The search string must be >= 3 characters long.");
					return false;
				}
		...
		addResult : function() {			
			...
			if (this.index < this.sites2search.length) {
				...
				a.appendChild(document.createTextNode( this.htmlDecode(this.sites2search[this.index][0]) ));
				a.href  = this.sites2search[this.index][1] + "?searchstring=" + this.replaceUmlauts(this.keyWords.join("&searchstring="), 1);
				a.title = this.sites2search[this.index][0];
				tr.appendChild(td);
				td.appendChild(a);
			}
			else if (!this.hasResult){
				td.appendChild(document.createTextNode("nothing found..."));
			}
			tr.appendChild(td);
		...
		updateStatus : function() {			
			if(tnode)
				tnode.nodeValue = parseInt(100.0/this.sites2search.length*this.index) + "% (" + this.index + " of " + this.sites2search.length + " pages)"
		},
		
		createResultTable : function() {
			var table = this.pEl.getElementsByTagName("table")[0];
			if (table)
				this.pEl.removeChild(table);
			...
			//th.appendChild(document.createTextNode("String found on page(s):"));
			//tr.appendChild(th);
			//thead.appendChild(tr);
			this.pEl.appendChild(table);
		...
		searchPage : function() {
			if (this.index == 0) {
				...
				{					
					img = new Image(43,11);
					img.src = "msearch/img/offlinesearchengine_loader.gif";
					img.id = "loader";
					img.alt = "bar";
					img.style.padding =  "2px 10px 0px 2px" 
				...
				if(!tnode)
				{
					tnode = document.createTextNode("0%");				
					tnode.id="statustext"
				}	
				tnode = this.pEl.insertBefore(tnode, this.pEl.getElementsByTagName("table")[0]);
			}
	...
	addContentLoadListener( function() { JSSearchEngine.init("<body>", "</body>", true, pages ); } );	